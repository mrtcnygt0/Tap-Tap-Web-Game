...
$sql = "SELECT score, task_twitter FROM users WHERE username = :username";
...
if ($result) {
    if ($result['task_twitter'] == 0) {
        $new_score = $result['score'] + 1000;
        $sql = "UPDATE users SET score=:new_score, task_twitter=1 WHERE username = :username";
        $stmt = $conn->prepare($sql);
